  ...
  run: async (client, interaction, args, prefix) => {
    ...
    if (!args[0])
      return interaction
        .reply({
          ephemeral: true,
          content: `👎 Please add the wished Music via: \`${prefix}playskip <Name/Link>\``,
        })
        .catch(() => null);
    ...
    try {
      ...
      // get playlist & song from the link
        ...
        playlist = await YouTube.getPlaylist(
          ConstantValues.urls.youtubPlaylist + isSong[2]
      ...
      if (!playlist) {
        ...
        return interaction
          .editReply({
            ephemeral: false,
            content: `▶️ **Now playing and skipping to: __${song.title}__** - \`${song.durationFormatted}\``,
          })
          .catch(() => null);
      } else {
        ...
        return interaction
          .editReply({
            ephemeral: false,
            content: `👍 **Now playing and skipping to: __${
              song.title
