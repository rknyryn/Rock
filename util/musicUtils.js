...
module.exports = (client) => {
  ...
  client.getTime = () => {
    const d = new Date();
    return `${m2(d.getHours())}:${m2(d.getMinutes())}:${m2(
      d.getSeconds()
    )}.${m3(d.getMilliseconds())}`;
  };

  client.getYTLink = (ID) => {
    return ConstantValues.urls.youtubVideo + ID;
  };
  ...
  client.getResource = (queue, songInfoId, seekTime = 0) => {
    ...
    const requestOpts = {
      ...
      encoderArgs: Qargs
        ? ["-af", Qargs]
        : ["-af", "bass=g=6,dynaudnorm=f=200"], // queue.filters
    };
    if (
      client.config.YOUTUBE_LOGIN_COOKIE &&
      client.config.YOUTUBE_LOGIN_COOKIE.length > 10
    ) {
    ...
    const resource = createAudioResource(
      dcYtdl(client.getYTLink(songInfoId), requestOpts),
      {
        inlineVolume: true,
      }
    );
